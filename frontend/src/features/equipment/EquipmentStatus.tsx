...
export function EquipmentStatus({ equipment, tumblerState, inProgress }: EquipmentStatusProps) {
  const state = tumblerState?.state ?? 'moving';
  const badge = STATE_BADGE[state];
  const isBlinking = inProgress && state === 'moving';
  ...
  return (
    ...
    <Card shadow="sm" padding="md" radius="md" withBorder className="w-full">
      <div className="flex items-center justify-between mb-2">
        ...
        <Badge color={badge.color} variant="filled" size="md" className={isBlinking ? 'animate-pulse' : ''}>
          {badge.label}
        </Badge>
      </div>
